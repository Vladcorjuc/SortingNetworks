...
public class Sequence {
    ...
    public static void main(String[] args) {
        for (int value = 0; value < Utils.POW_2[4+1]; value++) {
            Sequence input = new Sequence(Objects.requireNonNull(Sequence.getInstance(4, value)));
            System.out.println(input+ " " + input.isSorted());
        }
    }

    public List<Pair<Integer, Integer>> getBadPositions() {
        List<Pair<Integer,Integer>> pos = new ArrayList<>();
        for(int i=1;i<bitSet.length;i++){
            if(areSwappable(i-1,i)){
                pos.add(new Pair<>(i-1,i));
            }
        }
        return pos;
    }
